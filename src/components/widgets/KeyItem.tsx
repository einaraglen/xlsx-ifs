...
const KeyZone = (props: Props) => {
  ...
  return (
    <Fragment>
      ...
      <div className="flex items-center text-xs bg-zinc-900/30 shadow-lg px-3 rounded-full ring-1 ring-inset ring-white/10">{props.data.length == 0 ? <span className="text-zinc-400 italic">Blank</span> : <div className="truncate font-medium">{props.data}</div>}</div>
      <div className="flex items-center justify-center w-full">
        <HiOutlineSquare3Stack3D className={classNames(group ? "opacity-100" : "opacity-0", "text-emerald-500 transition-all")} />
      </div>
      <Toggle disabled={(selected && selected[props.data] == null) as boolean} value={group} setValue={setGroup} />
    </Fragment>
  );
};
