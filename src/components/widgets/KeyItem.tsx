...
const KeyZone = (props: Props) => {
  ...
  return (
    <Fragment>
      ...
      >
        <div className="w-32 truncate text-sm">{selected && selected[props.data]}</div>
      ...
      <div className="flex items-center text-sm">{props.data.length == 0 ? <span className="text-zinc-400 italic">Blank</span> : <div className="truncate font-semibold">{props.data}</div>}</div>
    </Fragment>
  );
};
